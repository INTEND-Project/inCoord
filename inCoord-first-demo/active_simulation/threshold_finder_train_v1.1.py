...
class LatencyOptimizationEnv(gym.Env):
    ...
    def __init__(self): #, data):
        ...
        # Load dataset
        # self.data = data
        # I don't know if I need the following element
        self.index = 0  # Track current sample
        self.step_count = 0
        self.max_steps_per_episode = 10000

        # Observed variables
        self._network_latency = -1
        self._computing_latency = -1


        # Define observation space
        self.observation_space = spaces.Dict(
            {
            "infrastructure": spaces.Dict({
                # source: Wei, Sheng, and Viswanathan Swaminathan. "Low latency live video streaming over HTTP 2.0." 2014.
                "network": spaces.Box(low=0, high=15, shape=(1,), dtype=np.float32),
                
                "computing": spaces.Box(low=0, high=45, shape=(1,), dtype=np.float32),
                }
            ),
            "application": spaces.Dict(
                {
                "video_quality": spaces.Discrete(7),
                "cost": spaces.Box(low=np.array([15000,]), high=np.array([90000,]))
                }
            )
            }
        )
        ...
        # 4 = Increase Network, 5 = Increase Computing, 6 = Increase Network - decrease computing, 
        # 7 = Increase Computing - decrease Network
